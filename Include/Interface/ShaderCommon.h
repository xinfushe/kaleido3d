...
K3D_COMMON_NS
{
  ...
  struct Uniform
  {
    Uniform() K3D_NOEXCEPT
      : VarType(EDataType::EUnknown)
      , VarName("")
      , VarOffset(0)
      , VarSzArray(0)
    {
  ...
  struct Binding
  {
    Binding() K3D_NOEXCEPT
      : VarType(EBindType::EUndefined)
      , VarName("")
      , VarStage(ES_Vertex)
      , VarNumber(0)
    {
    ...
    Binding(EBindType t, std::string n, EShaderType st, uint32 num)
      : VarType(t)
      , VarName(n.c_str())
      , VarStage(st)
      , VarNumber(num)
  ...
  KFORCE_INLINE BindingTable operator|(BindingTable const& a,
                                       BindingTable const& b)
  {
