...
// @see MTLRenderAttachmentDesc,
// VkAttachmentDescription, for renderpass creation
...
struct RenderViewportDesc
{
   ...
   * @param bufCount
   */
  RenderViewportDesc(uint32 width,
                     uint32 height,
                     EPixelFormat format,
                     EPixelFormat dsFormat,
                     bool hasDepth,
                     uint32 bufCount)
    : Width(width)
    , Height(height)
...
inline EGpuResourceAccessFlag
operator|(EGpuResourceAccessFlag const& lhs, EGpuResourceAccessFlag const& rhs)
{
...
inline EGpuResourceCreationFlag
operator|(EGpuResourceCreationFlag const& lhs,
...
/* in d3d12 and vulkan, each resource has a memory for resource foot print which
 * needs extra memory. */
struct ResourceDesc
{
...
inline ETextureAspectFlag
operator|(ETextureAspectFlag const& lhs, ETextureAspectFlag const& rhs)
{
