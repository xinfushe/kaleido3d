...
void CommandContext::SetPipelineState(uint32 hashCode, rhi::PipelineStateRef const& pipelineState)
{
	K3D_ASSERT(pipelineState);
  if (pipelineState->GetType() == rhi::EPSO_Compute)
  {
    ComputePipelineState * computePso = static_cast<ComputePipelineState*>(pipelineState.Get());
    vkCmdBindPipeline(m_CommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePso->m_Pipeline);
  }
  else
  {
    RenderPipelineState * gfxPso = static_cast<RenderPipelineState*>(pipelineState.Get());
...
void CommandContext::SetPipelineLayout(rhi::PipelineLayoutRef pRHIPipelineLayout)
{
	...
	vkCmdBindDescriptorSets(m_CommandBuffer, m_CmdType == rhi::ECMD_Graphics ? VK_PIPELINE_BIND_POINT_GRAPHICS : VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout->NativeHandle(), 0, 1, sets, 0, NULL);
}
